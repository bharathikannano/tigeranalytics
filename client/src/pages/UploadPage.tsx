// ─────────────────────────────────────────────────────────────────────────────
// pages/UploadPage.tsx  —  CSV upload form + upload history table
//
// Data flow:
//   1. User drops/selects a .csv file → local state
//   2. Clicks Upload → uploadMutation.mutate(file) → POST /api/pricing/upload
//   3. On success  → cache for 'upload-logs' is invalidated → history refetches
//   4. Upload history is fetched via useQuery(['upload-logs']) on page load
// ─────────────────────────────────────────────────────────────────────────────
...
// Small helper: renders a coloured status badge for a log entry
...
export default function UploadPage() {
  ...
  // Local UI state — only used for the upload form, not persisted
  ...
  // Fetch upload history — TanStack Query handles loading/error state for us
  ...
  // Upload mutation — replaces the manual try/catch boilerplate
  const uploadMutation = useMutation({
    mutationFn: (f: File) => uploadCsv(f, setProgress),

    onMutate: () => {            // called the moment mutate() is invoked
      setStatus('uploading');
      setProgress(0);
      setResult(null);
      setErrorMsg('');
    ...
    onSuccess: (res) => {        // called when the API returns 2xx
      setStatus('success');
      setResult({ rowsInserted: res.rowsInserted, fileName: res.fileName });
      setFile(null);
      // Tell TanStack Query the logs are stale → triggers a background refetch
      queryClient.invalidateQueries({ queryKey: ['upload-logs'] });
    },

    onError: (err) => {          // called when the API returns an error
      setStatus('error');
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Upload CSV files from retail stores. All rows are validated before saving.
        </p>
      </div>
      ...
      <div className="card p-4 flex gap-4 items-start bg-blue-50 border-blue-200">
        ...
        <div className="text-sm text-blue-800">
          ...
          <code className="block bg-white/70 rounded px-3 py-2 font-mono text-xs border border-blue-200">
            ...
            STORE-001,ABC-001,Widget Pro,9.99,2024-01-15
          </code>
          <p className="mt-2 text-xs text-blue-600">Headers are case-insensitive. Date: YYYY-MM-DD. Price: numeric.</p>
        </div>
      </div>
      ...
      <div className="card p-6 space-y-5">
        ...
        <UploadDropzone onFile={setFile} disabled={status === 'uploading'} />
          ...
          <div className="p-4 rounded-lg bg-red-50 border border-red-200">
            ...
            {errors.length > 0 && (
              <ul className="mt-2 max-h-48 overflow-y-auto space-y-0.5">
                {errors.map((e, i) => <li key={i} className="text-xs text-red-600 font-mono">• {e}</li>)}
              </ul>
            )}
          </div>
        )}
          ...
          {logsLoading ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['File Name', 'Rows', 'Status', 'Uploaded At'].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {logs.map((log: UploadLog) => (

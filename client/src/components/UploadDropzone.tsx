...
// Drag-and-drop file picker.  Accepts .csv files only.
// Props:
//   onFile(file)  — called with the selected File object
//   disabled      — disables all interaction (e.g. during upload)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function UploadDropzone({ onFile, disabled }: Props) {
  ...
  // Validate extension then forward the file to the parent
  ...
  return (
    <div
      // Visual cues: highlight border when dragging, dim when disabled
      ...
      onDrop={onDrop}
    >
      {/* Hidden native file input — triggered by clicking the whole zone */}
      <input
        ref={inputRef}
        type="file"
        accept={ACCEPTED}
      ...
      <p className="text-sm font-semibold text-gray-700">Drag &amp; drop a CSV file here, or click to browse</p>
      <p className="text-xs text-gray-500 mt-1">
        Required columns: <code className="bg-gray-100 px-1 rounded">Store ID, SKU, Product Name, Price, Date</code>
      </p>
      {fileError && <p className="mt-3 text-xs text-red-600 font-medium">{fileError}</p>}
    </div>
  );
}
